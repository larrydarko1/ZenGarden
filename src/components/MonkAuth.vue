<template>
  <div class="monk-auth" :class="{ 'is-ready': isReady }">
    ...
    <transition name="auth-fade" mode="out-in">
      <!-- LOGIN -->
      <form v-if="mode === 'login'" key="login" @submit.prevent="login" class="zen-form" role="form" aria-labelledby="login-title">
        <div class="zen-heading" id="login-title">{{ t('auth.login') }}</div>
        ...
        <button type="submit" :disabled="isLoading" class="auth-btn" style="--i:2" aria-label="Login to your account">
          <transition name="btn-content" mode="out-in">
            ...
            <span v-else key="loader" class="zen-loader">
              <svg width="20" height="20" viewBox="0 0 20 20">
                <circle cx="10" cy="10" r="8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-dasharray="40 60" opacity="0.4">
        ...
        <div class="zen-link-row" style="--i:3">
          <a href="#" @click.prevent="switchMode('register')" aria-label="Switch to registration form">{{ t('auth.switchToRegister') }}</a>
        </div>
        <div class="zen-link-row" style="--i:4">
          <a href="#" @click.prevent="switchMode('recovery')" aria-label="Reset password using recovery code">{{ t('auth.forgotPassword') }}</a>
        </div>
      ...
      <!-- REGISTER -->
      <form v-else-if="mode === 'register'" key="register" @submit.prevent="register" class="zen-form" role="form" aria-labelledby="register-title">
        <div class="zen-heading" id="register-title">{{ t('auth.register') }}</div>
        ...
        <button type="submit" :disabled="isLoading" class="auth-btn" style="--i:2" aria-label="Create your new account">
          <transition name="btn-content" mode="out-in">
        ...
        <div class="zen-link-row" style="--i:3">
          <a href="#" @click.prevent="switchMode('login')" aria-label="Switch to login form">{{ t('auth.switchToLogin') }}</a>
        </div>
      ...
      <!-- RECOVERY -->
      <form v-else-if="mode === 'recovery' && !recoverySuccess" key="recovery" @submit.prevent="resetPassword" class="zen-form" role="form" aria-labelledby="recovery-title">
        <div class="zen-heading" id="recovery-title">{{ t('auth.recoverPassword') }}</div>
        <div class="recovery-info">{{ t('auth.recoveryInfo') }}</div>

        <div class="field-group" style="--i:0">
          <input v-model="recoveryUsername" :disabled="isLoading" :placeholder="t('auth.username')" required autocomplete="username" />
        </div>
        <div class="field-group" style="--i:1">
          <input v-model="recoveryCode" :disabled="isLoading" :placeholder="t('auth.recoveryCode')" required />
        </div>
        <div class="field-group" style="--i:2">
          <input v-model="newRecoveryPassword" :disabled="isLoading" type="password" :placeholder="t('auth.newPassword')" required autocomplete="new-password" />
        </div>
        <div class="field-group" style="--i:3">
          <input v-model="confirmRecoveryPassword" :disabled="isLoading" type="password" :placeholder="t('auth.confirmNewPassword')" required autocomplete="new-password" />
        </div>
      ...
      <div v-else-if="recoverySuccess" key="recovery-success" class="zen-form recovery-success">
        ...
        <button @click="goToLogin" class="auth-btn">{{ t('auth.goToLogin') }}</button>
      </div>
    </transition>
  </div>
</template>
